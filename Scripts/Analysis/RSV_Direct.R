...
write.tree(coverage.nj, "~/src/Masters/Project/Trees/rsv_nj_direct_tree.nwk")

coverage.nj.ggtree <- ggtree(coverage.nj)  %<+% metadata.gg
coverage.nj.ggtree + 
  geom_tiplab(size=3, offset=0) + 
  geom_tippoint(size=1, aes(color=individuals)) +
  labs(title = "RSV Neighbour Joining Tree", color="") +
  scale_color_manual(values=palette) +
  theme_tree(legend.position='bottom')

ggsave(paste(output_dir, "RSV_NJ.png", sep=""),
       height=5, 
       width=13, 
       dpi=300)
dev.off()
...
coverage.hclust.ggtree <- ggtree(coverage.clust) %<+% metadata.gg
coverage.hclust.ggtree + 
  geom_tiplab(size=3, offset=0) + 
  geom_tippoint(size=1, aes(color=individuals)) +
  labs(title = "RSV Hierachical Clustering Tree", color="Individuals") +
  scale_color_manual(values=palette) +
  theme_tree(legend.position='left')

ggsave(paste(output_dir, "RSV_HClust.png", sep=""),
       height=5, 
       width=14, 
       dpi=400)
dev.off()